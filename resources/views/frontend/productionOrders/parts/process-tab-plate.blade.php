...
<div class="model-products pt-3">
                    ...
                    @foreach ($items as $detail)
                        ...
                        <div class="item-product row item-product-status-{{ $po_status }}">
                            ...
                            <div class="col row justify-content-center align-self-center">
                                <div class="col-6">
                                    <label for="productCheckPlate-{{ $product->id }}" class="text-sm mb-0">
                                        Plate
                                    </label>
                                    ...
                                    <div class="form-check">
                                        <input
                                            ...
                                            type="checkbox"
                                            name="product[{{ $product->id }}][plate]"
                                            value="1"
                                            id="productCheckPlate-{{ $product->id }}"
                                            {{ !$detail->plate ? '' : ' checked' }}
                                        >

                                        <label class="form-check-label" for="productCheckPlate-{{ $product->id }}">
                                            Sudah cetak plate
                                        </label>
                                    </div>
                                </div>
    ...
    <div class="d-flex justify-content-end mt-4">
        <a href="#model-tab-1" class="btn btn-light modelTabs-nav">
            Sebelumnya
        </a>

        <div class="flex-grow-1"></div>

        <button type="submit" class="btn btn-default mr-3 border">
            Simpan
        </button>

        <a href="#model-tab-3" class="btn btn-info modelTabs-nav">
            Selanjutnya
        </a>
    </div>
</div>
