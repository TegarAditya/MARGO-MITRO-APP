        ...
        <div class="col-md-12">
            @can('salesperson_create')
                <div style="margin-bottom: 10px;" class="row">
                    <div class="col-lg-12">
                        <a class="btn btn-success" href="{{ route('frontend.salespeople.create') }}">
                                    ...
                                    <tr data-entry-id="{{ $salesperson->id }}">
                                        ...
                                        <td>
                                            @can('salesperson_show')
                                                <a class="btn btn-xs btn-primary" href="{{ route('frontend.salespeople.show', $salesperson->id) }}">
                                                    {{ trans('global.view') }}
                                                </a>
                                            @endcan

                                            @can('salesperson_edit')
                                                <a class="btn btn-xs btn-info" href="{{ route('frontend.salespeople.edit', $salesperson->id) }}">
                                                    {{ trans('global.edit') }}
                                                </a>
                                            @endcan

                                            @can('salesperson_delete')
                                                <form action="{{ route('frontend.salespeople.destroy', $salesperson->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('salesperson_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('frontend.salespeople.massDestroy') }}",
