    ...
    <div class="row justify-content-center">
        <div class="col-md-12">
            ...
            <div class="card">
                ...
                <div class="card-body">
                    <form method="POST" action="{{ route("frontend.stock-adjustments.update", [$stockAdjustment->id]) }}" enctype="multipart/form-data">
                        @method('PUT')
                        @csrf
                        <div class="form-group">
                            <label class="required" for="date">{{ trans('cruds.stockAdjustment.fields.date') }}</label>
                            <input class="form-control date" type="text" name="date" id="date" value="{{ old('date', $stockAdjustment->date) }}" required>
                            @if($errors->has('date'))
                        ...
                        <div class="form-group">
                            ...
                            <select class="form-control select2" name="product_id" id="product_id" required>
                                @foreach($products as $id => $entry)
                                    <option value="{{ $id }}" {{ (old('product_id') ? old('product_id') : $stockAdjustment->product->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group">
                            ...
                            <input class="form-control" type="number" name="quantity" id="quantity" value="{{ old('quantity', $stockAdjustment->quantity) }}" step="1" required>
                        ...
                        <div class="form-group">
                            ...
                            <textarea class="form-control" name="note" id="note">{{ old('note', $stockAdjustment->note) }}</textarea>
                            @if($errors->has('note'))
                                <div class="invalid-feedback">
                                    {{ $errors->first('note') }}
                                </div>
