...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.priceDetail.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.price-details.update", [$priceDetail->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('sales') ? 'is-invalid' : '' }}" name="sales_id" id="sales_id" required>
                    @foreach($sales as $id => $entry)
                        <option value="{{ $id }}" {{ (old('sales_id') ? old('sales_id') : $priceDetail->sales->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
                @if($errors->has('sales'))
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('price') ? 'is-invalid' : '' }}" name="price_id" id="price_id" required>
                    @foreach($prices as $id => $entry)
                        <option value="{{ $id }}" {{ (old('price_id') ? old('price_id') : $priceDetail->price->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
                @if($errors->has('price'))
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('diskon') ? 'is-invalid' : '' }}" type="number" name="diskon" id="diskon" value="{{ old('diskon', $priceDetail->diskon) }}" step="0.01" required max="100">
