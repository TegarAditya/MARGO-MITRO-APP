        ...
        <form method="POST" action="{{ route("admin.price-details.update", [$priceDetail->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('price') ? 'is-invalid' : '' }}" name="price_id" id="price_id" required>
                    @foreach($prices as $price)
                        <option data-price="{{ $price->price }}" value="{{ $price->id }}" {{ (old('price_id') ? old('price_id') : $priceDetail->price->id ?? '') == $price->id ? 'selected' : '' }}>{{ $price->nama_harga }}</option>
                    @endforeach
                </select>
                @if($errors->has('price'))
            ...
            <div class="form-group">
                ...
                <x-admin.form-group
                    ...
                    containerClass=" m-0"
                    boxClass=" px-2 py-0"
                    value="{{ old('diskon', $priceDetail->diskon) }}"
                    min="1"
                >
                    <x-slot name="left">
            ...
            <div class="form-group">
                ...
                <x-admin.form-group
                    ...
                    value="{{ old('custom_price', $priceDetail->custom_price) }}"
                    min="0"
                >
                <x-slot name="left">
