...
<div class="card">
    ...
    <div class="card-body">
        @if ($preorder->id)
            <div class="row mb-4">
                <div class="col">
                    ...
                    <div class="row">
                        <div class="col-auto">
                            ...
                            <p class="m-0">
                                <strong>{{ $preorder->productionperson->name ?? '-' }}</strong>
                            </p>
                        </div>
                        ...
                        <div class="col-auto ml-3">
                            ...
                            <p class="m-0">
                                <strong>{{ ucfirst($preorder->type) }}</strong>
                            </p>
                        </div>

                        <div class="col-auto d-flex align-items-center ml-3">
                            <a href="{{ route('admin.preorders.show', [
                ...
                <div class="col-auto text-right">
                    <h6 class="h6 text-primary m-0">{{ $preorder->date }}</h6>
                </div>
            </div>
        @endif
        ...
        <form method="POST" action="{{ !$preorder->id ? route('admin.preorders.store') : route("admin.preorders.update", [$preorder->id]) }}" enctype="multipart/form-data" id="modelForm">
            @method(!$preorder->id ? 'POST' : 'PUT')
            @csrf

            @php
            $tabs = [
                [ 'label' => 'Detail Order', 'enabled' => true ],
                // [ 'label' => 'Finishing', 'enabled' => !!$preorder->id ],
            ];
            @endphp
            ...
            <div class="tab-content" id="modelTabsContent">
                <div class="tab-pane fade show active" id="model-tab-1" role="tabpanel">
                    @include('admin.preorders.parts.tab-order')
                </div>
            </div>
        </form>
    </div>
