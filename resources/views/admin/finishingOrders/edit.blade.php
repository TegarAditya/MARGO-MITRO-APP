...
<div class="card">
    ...
    <div class="card-body">
        @if ($finishingOrder->id)
            <div class="row mb-4">
                <div class="col">
                    <h4>PO No. #{{ $finishingOrder->po_number }}</h4>
        ...
        <form method="POST" action="{{ !$finishingOrder->id ? route('admin.finishing-orders.store') : route("admin.finishing-orders.update", [$finishingOrder->id]) }}" enctype="multipart/form-data" id="modelForm">
            @method(!$finishingOrder->id ? 'POST' : 'PUT')
            @csrf

            @php
            $tabs = [
                [ 'label' => 'Detail Order', 'enabled' => true ],
                [ 'label' => 'Realisasi', 'enabled' => !!$finishingOrder->id ],
            ];
            @endphp
            ...
            <div class="tab-content" id="modelTabsContent">
                <div class="tab-pane fade show active" id="model-tab-1" role="tabpanel">
                    @include('admin.finishingOrders.parts.tab-order')
                </div>

                <div class="tab-pane fade" id="model-tab-2" role="tabpanel">
                    @include('admin.finishingOrders.parts.tab-realisasi')
                </div>
            </div>
        </form>
    </div>
