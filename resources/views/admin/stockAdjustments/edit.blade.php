...
<div class="card">
    ...
    <div class="card-body">
        ...
        @endif
        {{-- <form method="GET" action="{{ !$stockAdjustment->id ? route("admin.stock-adjustments.create") : route("admin.stock-adjustments.update", [$stockAdjustment->id]) }}" enctype="multipart/form-data" id="adjustmentForm"> --}}
        <form method="POST" action="{{ !$stockAdjustment->id ? route("admin.stock-adjustments.store") : route("admin.stock-adjustments.update", [$stockAdjustment->id]) }}" enctype="multipart/form-data" id="adjustmentForm">
            @method(!$stockAdjustment->id ? 'POST' : 'PUT')
            @csrf
            <div class="row">
                <div class="col-6">
            ...
            <div class="row mt-4">
                <div class="col-auto">
                    <button type="submit" class="btn btn-primary">Simpan Adjustment</a>
                </div>
                <div class="col"></div>
            </div>
        </form>
            ...
            <div class="modal-body">
                ...
                <div class="product-select" style="display: {{ !$products->count() ? 'none' : 'block' }}">
                    @foreach ($modal['items'] as $product)
                        ...
                        $selected = $stockAdjustment->details->where('product_id', $product->id)->count();
                        @endphp
                        <a
