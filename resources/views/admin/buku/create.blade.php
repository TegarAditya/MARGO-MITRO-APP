...
<div class="card">
    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.buku.store") }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                <label class="required" for="kelas">{{ trans('cruds.buku.fields.kelas') }}</label>
                <div style="padding-bottom: 4px">
                    <span class="btn btn-info btn-xs select-all" style="border-radius: 0">{{ trans('global.select_all') }}</span>
                    <span class="btn btn-info btn-xs deselect-all" style="border-radius: 0">{{ trans('global.deselect_all') }}</span>
                </div>
                <select class="form-control select2 {{ $errors->has('kelas') ? 'is-invalid' : '' }}" name="kelas[]" id="kelas" required multiple>
            ...
            <div class="form-group">
                <label class="required" for="semester_id">{{ trans('cruds.buku.fields.semester') }}</label>
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('tipe_pg') ? 'is-invalid' : '' }}" name="tipe_pg" id="tipe_pg">
                    ...
                    @foreach(App\Models\Product::TIPE_PG_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('tipe_pg', 'non_pg') === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
