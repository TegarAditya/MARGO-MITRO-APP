...
<div class="card">
    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.production-order-details.update", [$finishingOrderDetail->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('finishing_order_id') ? 'is-invalid' : '' }}" name="finishing_order_id" id="finishing_order_id">
                    @foreach($finishing_orders as $id => $entry)
                        <option value="{{ $id }}" {{ (old('finishing_order_id') ? old('finishing_order_id') : $finishingOrderDetail->finishing_order->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('product') ? 'is-invalid' : '' }}" name="product_id" id="product_id">
                    @foreach($products as $id => $entry)
                        <option value="{{ $id }}" {{ (old('product_id') ? old('product_id') : $finishingOrderDetail->product->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('order_qty') ? 'is-invalid' : '' }}" type="number" name="order_qty" id="order_qty" value="{{ old('order_qty', $finishingOrderDetail->order_qty) }}" step="1" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('prod_qty') ? 'is-invalid' : '' }}" type="number" name="prod_qty" id="prod_qty" value="{{ old('prod_qty', $finishingOrderDetail->prod_qty) }}" step="1" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('ongkos_satuan') ? 'is-invalid' : '' }}" type="number" name="ongkos_satuan" id="ongkos_satuan" value="{{ old('ongkos_satuan', $finishingOrderDetail->ongkos_satuan) }}" step="0.01" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('ongkos_total') ? 'is-invalid' : '' }}" type="number" name="ongkos_total" id="ongkos_total" value="{{ old('ongkos_total', $finishingOrderDetail->ongkos_total) }}" step="0.01" required>
