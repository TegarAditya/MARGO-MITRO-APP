...
<div class="card">
    ...
    <div class="card-body">
        ...
        @endif
        <form method="POST" action="{{ !$purchase->id ? route("admin.purchases.store") : route("admin.purchases.update", [$purchase->id]) }}" enctype="multipart/form-data" id="purchaseForm">
            @method(!$purchase->id ? 'POST' : 'PUT')
            @csrf
            <div class="row">
                <div class="col-6">
                    <div class="form-group">
                        <label for="no_spk">No. Produk Masuk</label>
                        <input class="form-control h-auto py-1 {{ $errors->has('no_spk') ? 'is-invalid' : '' }}" type="text" name="no_spk" id="no_spk" value="{{ old('no_spk', $purchase->no_spk) }}" readonly placeholder="(Otomatis)">
                ...
                <div class="col-6">
                    <div class="form-group">
                        <label class="required" for="semester_id">Semester</label>
                        <select class="form-control select2 {{ $errors->has('semester_id') ? 'is-invalid' : '' }}" name="semester_id" id="semester_id">
                            @foreach($semesters as $id => $entry)
                                <option value="{{ $id }}" {{ (old('semester_id') ? old('semester_id') : $purchase->semester_id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                            @endforeach
                        </select>
                ...
                <div class="col-6">
                    <div class="form-group">
                        <label class="required" for="productionperson_id">Subkontraktor</label>
                        <select class="form-control select2 {{ $errors->has('productionperson') ? 'is-invalid' : '' }}" name="productionperson_id" id="productionperson_id" required>
                            @foreach($subkontraktors as $id => $entry)
                                <option value="{{ $id }}" {{ (old('productionperson_id') ? old('productionperson_id') : $purchase->productionperson_id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                            @endforeach
                        </select>
                ...
                <div class="col-6">
                    <div class="form-group">
                        ...
                        <input class="form-control {{ $errors->has('no_suratjalan') ? 'is-invalid' : '' }}" type="text" name="no_suratjalan" id="no_suratjalan" value="{{ old('no_suratjalan', $purchase->no_suratjalan) }}">
                ...
                <div class="col-12">
                    <div class="form-group">
                        ...
                        <textarea class="form-control {{ $errors->has('note') ? 'is-invalid' : '' }}" name="note" id="note">{{ old('note', $purchase->note) }}</textarea>
                        @if($errors->has('note'))
            ...
            <div class="row">
                <div class="col-6">
                    <div class="form-group">
                        <button type="submit" class="btn btn-primary" name="filter" value="filter">Filter</a>
                    </div>
                </div>
            </div>
            ...
            [
                ...
                <div class="product-list-group">
                    ...
                    <div class="product-list">
                        @include('admin.purchases.parts.item-product', [
                        ...
                        @if ($item['items']->count())
                            ...
                            @endphp
                            @each('admin.purchases.parts.item-product', $sortedProducts, 'detail')
                        @endif
                    </div>
            ...
            <div class="row mt-4">
                <div class="col-auto">
                    <button type="submit" class="btn btn-primary">Simpan Produk Masuk</a>
                </div>
            </div>
        </form>
    </div>
            ...
            <div class="modal-body">
                ...
                <div class="product-select" style="display: {{ !$products->count() ? 'none' : 'block' }}">
                    @foreach ($modal['items'] as $product)
                        ...
                        $selected = $purchase->details->where('product_id', $product->id)->count();
                        @endphp
                        <a
