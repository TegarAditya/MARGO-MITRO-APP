...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.customPrice.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.custom-prices.update", [$customPrice->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                <label class="required" for="nama">{{ trans('cruds.customPrice.fields.nama') }}</label>
                <input class="form-control {{ $errors->has('nama') ? 'is-invalid' : '' }}" type="text" name="nama" id="nama" value="{{ old('nama', $customPrice->nama) }}" required>
                @if($errors->has('nama'))
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('sales') ? 'is-invalid' : '' }}" name="sales_id" id="sales_id" required>
                    @foreach($sales as $id => $entry)
                        <option value="{{ $id }}" {{ (old('sales_id') ? old('sales_id') : $customPrice->sales->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
                @if($errors->has('sales'))
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('kategori') ? 'is-invalid' : '' }}" name="kategori_id" id="kategori_id" required>
                    @foreach($kategoris as $id => $entry)
                        <option value="{{ $id }}" {{ (old('kategori_id') ? old('kategori_id') : $customPrice->kategori->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
