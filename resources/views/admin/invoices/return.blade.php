...
<div class="card">
    ...
    <div class="card-body">
        ...
        <div class="tab-invoice">
            <form method="GET" action="{{ route('admin.invoices.retur') }}" enctype="multipart/form-data" id="salesmanForm">
                <div class="form-group">
                    <label class="required" for="salesman">Salesman</label>
                    <select class="form-control select2 {{ $errors->has('salesman') ? 'is-invalid' : '' }}" name="salesman" id="salesman" required>
                        @foreach($sales as $id => $entry)
                            <option value="{{ $id }}" {{ request('salesman') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                        @endforeach
                    </select>
            ...
            <form method="POST" action="{{ route('admin.invoices.saveretur') }}" enctype="multipart/form-data" id="invoiceForm">
                @csrf
                <input type="hidden" name="nominal" value="" />
                <input type="hidden" name="salesman_id" value="{{ request('salesman', '') }}" />
                ...
                <div class="form-group">
                    ...
                    <input class="form-control {{ $errors->has('no_invoice') ? 'is-invalid' : '' }}" type="text" name="no_invoice" id="no_invoice" value="{{ old('no_invoice', '') }}" readonly placeholder="(Otomatis)">
                ...
                <div class="product-summary" style="display: 'none'">
                    <div class="row border-top pt-2">
                        <div class="col text-right">
                            <p class="mb-0">
                                <span class="text-sm">Grand Total</span>
                                <br />
                                <strong class="product-total">@money(0)</strong>
                            </p>
                        </div>
                    </div>
                </div>
    ...
    $(function() {
        ...
        productSelectItems.on('click', function(e) {
            ...
            product.find('.product-price_text').val(numeral(price != 0 ? price : data.price).format('0,0'))
                ...
                .attr('min', 1)
                .attr('required', true);
            product.find('.product-order').val(data.order)
                .attr('name', name+'['+data.id+'][order_id]');
